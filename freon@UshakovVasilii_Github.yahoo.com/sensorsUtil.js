...
export default class SensorsUtil extends CommandLineUtil {
        ...
        super.execute(() => {
            ...
            try {
                ...
                try {
                    ...
                    let lineRemoved = this._output
                        .filter((l) => l.trim() !== ",")
                        .join("\n");
                    let errorRemoved = lineRemoved.replace(
                        /ERROR.*Can't read/,
                        ""
                    );
                    errorRemoved = errorRemoved.replace(
                        /ERROR.*I\/O error/,
                        ""
                    );
    ...
    _parseSensorsOutput(sensorFilter, sensorType, sensorFamily) {
        ...
        for (var chipset in data) {
            ...
            if (
                !data.hasOwnProperty(chipset) ||
                (gpuFamily != gpuFilter.test(chipset) &&
                    (tempType || powerType))
            )
                continue;
            ...
            if (
                !data.hasOwnProperty(chipset) ||
                (diskFamily != diskFilter.test(chipset) && tempType)
            )
                continue;
