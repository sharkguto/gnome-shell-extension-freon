...
const Async = {
    ...
    map(
        arr,
        mapClb /* function(in, successClb)) */,
        resClb /* function(result) */
    ) {
        ...
        for (let i = 0; i < arr.length; ++i) {
            ...
            ); // i needs to be bound since it will be changed during the next iteration
        }
    },
};
                ...
                try {
                    ...
                    Async.map(
                        ...
                        function (obj, callback) {
                            ...
                            let driveProxy = new UDisksDriveProxy(
                                Gio.DBus.system,
                                "org.freedesktop.UDisks2",
                                obj,
                                function (res, error) {
                                    if (error) {
                                        //very unlikely - we even checked the interfaces before!

                                        logError(
                                            "[FREON] Could not create proxy on " +
                                                obj +
                                                ":" +
                                                error
                                        );
                                    ...
                                    let ataProxy = new UDisksDriveAtaProxy(
                                        Gio.DBus.system,
                                        "org.freedesktop.UDisks2",
                                        obj,
                                        function (res, error) {
                    ...
                    logError("[FREON] Could not find UDisks2 objects: " + e);
                }
            }
        );
    }
